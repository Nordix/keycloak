...
/**
 * WWW-Authenticate challenge returned by the UserInfo endpoint. The challenge is built by the same
 * helper the admin REST API uses, so these tests guard it against changes made for the admin API.
 */
...
public class UserInfoBearerChallengeTest {
    ...
    public void expiredToken() throws IOException {
        ...
        try (SimpleHttpResponse response = userInfoGet(token)) {
            assertThat(response.getStatus(), is(401));
            // This endpoint reports all verification failures the same way
